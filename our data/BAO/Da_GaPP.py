...
if __name__=="__main__":
    ...
    ax.set_ylabel(r"$DA(z)$", fontsize=22)
    plt.xlim(zmin, 2.234)
    ...
    plt.legend((r"$DA(z)$ rec ($1\sigma$)", "$DA(z)$ rec ($2\sigma$)", "$DA(z)$ rec ($3\sigma$)", "$DA(z)$ data"), fontsize='22', loc='upper left')
    plt.savefig('DA_z.pdf', bbox_inches='tight')
    plt.show()
    # =========== saving the plot
