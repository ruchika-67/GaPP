...
if __name__=="__main__":
    ...
    ax.set_ylabel(r"$DA(z)/rd$", fontsize=22)
    plt.xlim(zmin, 2.234)
    ...
    plt.legend((r"$DA(z)/rd$ rec ($1\sigma$)", "$DA(z)/rd$ rec ($2\sigma$)", "$DA(z)/rd$ rec ($3\sigma$)", "$DA(z)/rd$ data"), fontsize='22', loc='upper left')
    plt.savefig('DAbyrd_z.pdf', bbox_inches='tight')
    plt.show()
    # =========== saving the plot
