...
if __name__=="__main__":
    ...
    filename1 = 'Hdata'
    filename2 = 'Hsigma'
    (Z,Hz) = loadtxt(filename1+'.dat',unpack='True')
    (Sigma) = loadtxt(filename2+'.dat',unpack='True')
    ...
    plt.errorbar(Z, Hz,  fmt='o', color='black')
    ...
    plt.legend((r"$E(z)$ rec ($1\sigma$)", "$E(z)$ rec ($2\sigma$)", "$E(z)$ rec ($3\sigma$)", "$E(z)$ data"), fontsize='22', loc='upper left')
    plt.savefig('Ez.pdf', bbox_inches='tight')
    plt.show()
    # =========== saving the plot
    fig.savefig(filename1+'_reconst.png')
