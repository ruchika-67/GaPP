...
if __name__=="__main__":
    ...
    ax.set_ylabel(r"$Om(z)$", fontsize=22)
    plt.xlim(zmin, zmax)
...
#    plt.legend((r"$f(z = 1) (\Lambda CDM)$",r"$f(z)$ rec ($1\sigma$)", "$f(z)$ rec ($2\sigma$)", "$f(z)$ rec ($3\sigma$)", "$f(z)$ data"), fontsize='22', loc='lower left')
    plt.savefig('om.pdf', bbox_inches='tight')
    plt.show()
    # =========== saving the plot
